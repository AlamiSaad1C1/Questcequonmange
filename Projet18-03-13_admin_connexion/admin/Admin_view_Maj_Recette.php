	...
	<form action="index.php?action=Admin_trait__Maj_Recette" method="post" enctype="multipart/form-data">
		...
		echo '</div>'; // fin de <div class="un">
			...
			// 1. on remplit un tab ViandesPoissonsnom à partir des noms des viandes et poissons de la BDD
		            ...
		            $sql="SELECT * FROM ingredient WHERE type='Viandes & poissons' ORDER BY nom";          
		            $q=$pdo->prepare($sql);
		            $q->execute();
		            while($line=$q->fetch()) {
		            ...
		            // légumes
		            $Legumesnom=array();
		            $sql="SELECT * FROM ingredient WHERE type='legume' ORDER BY nom";          
		            $q=$pdo->prepare($sql);
		            $q->execute();
		            while($line=$q->fetch()) {
		            ...
		            //féculents
		            $Feculentnom=array();
		            $sql="SELECT * FROM ingredient WHERE type='feculent' ORDER BY nom";          
		            $q=$pdo->prepare($sql);
		            $q->execute();
		            while($line=$q->fetch()) {
